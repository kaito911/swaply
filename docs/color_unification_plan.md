> ⚠️ **STALE — 参照禁止（2026-07-26）**
> 本ドキュメントは旧 indigo `#4B3BD6` 統一案であり、**現行方針と逆行している**。
> その後の「わくわく化 STEP1」で primary は coral `#D94370` に確定し、navy は撤去済み
> （根拠: constants/theme.ts:19-40）。本書の Phase 1（primary→#4B3BD6 / #6D5EF5→primary）を
> 実行すると STEP1 の coral 化を無自覚に巻き戻す。色統一の実装時に本書を根拠にしないこと。
> 現行の残響紫タスクは取引詳細画面の colors.* 置換（新定数不要・OTA可）で扱う。
