...
最終更新: 2026-05-09 / バージョン: v1.8 / 前提: feat/trade-pr1 ブランチ HEAD = 5426c24 (refactor_plan v1.6 直後、v1.7 はスキップ)
...
## 3.10 マスタデータ整備計画 (v1.8 新設)

3 候補 (鬼滅 + コナン + サンリオ) のマスタデータを β 前に整備。

### 鬼滅の刃

#### キャラ (主要 30+)

主要キャラ: 竈門炭治郎、竈門禰豆子、我妻善逸、嘴平伊之助、冨岡義勇、胡蝶しのぶ、煉獄杏寿郎、宇髄天元、時透無一郎、甘露寺蜜璃、伊黒小芭内、不死川実弥、悲鳴嶼行冥、産屋敷耀哉

柱 9 名 + 隊員主要 10 名 + 鬼 (鬼舞辻無惨、上弦の鬼 6 名、下弦の鬼 6 名) = 約 30+

#### 主要グッズシリーズ

- ufotable Cafe コラボグッズ (年複数回)
- ホログラム缶バッジ全 16 種シリーズ (場面写ベース)
- トレーディングアクリルカード全 16 種
- アクマカチャーム
- 劇場版「無限城編」連動グッズ (第二章 2026 公開予定)
- ジャンプフェスタ 2026 限定品
...
主要キャラ: 江戸川コナン、毛利蘭、毛利小五郎、灰原哀、安室透 (降谷零)、赤井秀一、怪盗キッド、服部平次、遠山和葉、世良真純、白馬探、京極真、佐藤美和子、高木渉、目暮十三、鈴木園子、鈴木次郎吉

少年探偵団 4 名 + 主要 10 名 + 警察 5 名 + 黒の組織 5 名 + 怪盗キッド + 安室・赤井他 5 名 = 約 30+

#### 主要グッズシリーズ

- 毎春劇場版グッズ (2026 第 29 弾「ハイウェイの堕天使」4/10 公開)
- 名探偵コナンカフェ 2026 (BOX cafe&space GEMS 渋谷店 4/10-6/28、グランドスケープ池袋店 4/17-8/2)
- コナンプラザ全国巡回
- 場面写トレーディング缶バッジ
- 歴代場面写シリーズ (劇場版 28 弾分の蓄積)
- ホログラム缶バッジ
- ミニアクスタ・クリアカード
...
主要キャラ (大賞 2025 上位 10): ポムポムプリン (1位)、シナモロール (2位)、ポチャッコ (3位)、クロミ (4位)、ハローキティ (5位)、マイメロディ (6位)、リトルツインスターズ (7位)、ハンギョドン (8位)、あひるのペックル (9-10位圏)、けろけろけろっぴ、バッドばつ丸、タキシードサム、ぐでたま、シナモロール

#### 主要グッズシリーズ

- サンリオ当りくじ (毎月複数キャラ展開、ファミマ・サンリオショップ、A-F 賞 + ラストワン)
- ガシャ サンリオキャラクターズ各種 (バンダイ毎月、5-10 種)
- アクスタ・缶バッジ・キーホルダー・マスコットのブラインドガチャ (年数十シリーズ)
- ピューロランド限定グッズ
- サンリオフェス 2026 みなとみらい (6/27-28) 限定品
- IP × IP コラボ (推しの子 × サンリオ 2024-2025、TWICE LOVELYS × サンリオ 2025、フラガリアメモリーズ等)

#### 公式情報源

- 公式 IG @sanrio_official_jp / @sanrio_ec_official
- 公式 X @sanrio_news
- キャラ別: @kuromi_project / @i_cn_official / @purin_sanrio 等
- Sanrio+ アプリ
- ピューロランド公式

### マスタデータ整備工数見積もり

| 候補 | キャラ数 | 主要グッズシリーズ数 | 工数 (キャラ + シリーズ + 公式画像 URL 収集) |
|---|---|---|---|
| 鬼滅の刃 | 30+ | 10-15 | 6-10h |
| 名探偵コナン | 30+ | 8-12 | 6-10h |
| サンリオ | 10-15 (Phase 1 主要) | 15-20 (年間展開多) | 8-12h |
| **合計** | **70-75** | **33-47** | **20-32h** |

**進め方**:
- 公式情報源から CSV 形式で収集 (キャラ名、画像 URL、シリーズ名、発売日、価格、種類数)
- Supabase に `master_works` / `master_characters` / `master_item_series` テーブル作成 (キャラ・シリーズマスタ)
- UI 層で出品時の autocomplete として活用
- **β 前必須**: 鬼滅 30 キャラ + コナン 30 キャラ + サンリオ 10-15 キャラの最低限 = 約 75 件のキャラマスタ
- **β 後拡張**: 各キャラのバリエーション (制服 ver / 私服 ver 等)、Phase 2 拡張作品

### マスタデータ整備のリスク

| 項目 | リスク | 対策 |
|---|---|---|
| キャラ名表記揺れ (例: 「コナン」vs「江戸川コナン」) | 検索失敗で UX 低下 | canonical name + aliases の 2 軸構造 (M-search の members.ts と同設計) |
| グッズシリーズの公式画像著作権 | β 期間中の利用範囲確認必要 | アニプレックス・小学館・サンリオの利用規約確認、必要なら placeholder 画像で運用 |
...
### 4.0 β リリース前必須項目 (累計 138-194h、約 17-24 日) — v1.8 で更新

β 開始 (2026-06) までに**必ず**完了する項目。v1.5 で Phase S + 法的整備、v1.6 で C-S3 Vision API、v1.8 で **DB schema 移行 + マスタデータ整備 + 会場モード対応** を追加。

| Order | 項目 | 工数 | Phase |
|---|---|---|---|
| 1 | **DB schema 移行** (cards → items 拡張、章 3.9) ← v1.8 新設 | **7.5-11.5h** | **β 前必須 / 最優先** |
| 2 | **マスタデータ整備** (鬼滅 + コナン + サンリオ、章 3.10) ← v1.8 新設 | **20-32h** | **β 前必須 / DB 後** |
| 3 | UI-7 出品画面改修 (items 対応 + 状態/配送/Storage 含、scope 増) | **22-28h** (旧 19-23h、items 対応で +3-5h) | Phase B |
| 4 | UI-8〜11 (タイポ + 余白 + 質感 + 写真 + 残り hex) | 6-9h | Phase B |
| 5 | **会場モード対応** (ufotable Cafe / コナンカフェ / サンリオショップ・ピューロランド の venue 拡張) ← v1.8 新設 | **6-10h** | **Phase B / M** |
| 6 | Phase M メッセージ + Realtime + 通知 + 会場リアルタイム | 28-40h | Phase M |
| 7 | **Phase S 詐欺対策** (発送日合意 + 証跡 + Vision API + 自動監視 + 仲裁) ← v1.5 新設 / v1.6 で +4h | 20-27h | Phase S |
| 8 | Phase 2 早期: エスクロー + 72h + 追跡番号 | 8-15h | Phase 2 早期 |
| 9 | **法的整備** (利用規約 + プライバシーポリシー + 特商法表記) ← v1.5 新設 | 10-20h | Phase 2 並行 |
| 10 | (Phase M 内) 個人情報マスキング | (M-4 含む) | Phase M |

**累計 138-194h** (旧 91-131h から +47-63h: DB 移行 + マスタデータ + UI-7 scope 増 + 会場モード)。

**進行順序** (v1.8 更新): **DB 移行 → マスタデータ整備 → UI-7 (items 対応) → UI-8〜11 → 会場モード → Phase M → Phase S → Phase 2 早期** の直列推奨。
法的整備は弁護士レビュー必要 + コンテンツ作成中心のため B/M/S と並行進行可。

**Phase 順序の再評価 (v1.8)**:
- v1.6 までは UI-7 が Phase B 最初の大型タスクだった
- v1.8 では **DB 移行 → マスタデータ整備が最優先** (UI-7 が items 対応するためには schema が先)
- UI-7 自体の scope が +3-5h 増 (category 別 layout 切替、autocomplete 等)
- 会場モード対応は元々 Phase M のリアルタイム化に統合予定だったが、ufotable Cafe / コナンカフェ等の **アニメ系イベント venue を扱う必要** が出たため独立タスク化
- Phase M / Phase S の設計は変更なし (Vision API/pg_cron/メッセージ機能等は category 非依存)
