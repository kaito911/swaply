...
最終更新: 2026-05-07 / バージョン: v1.5 / 前提: feat/trade-pr1 ブランチ HEAD = 6440c94 (refactor_plan v1.4 直後)
...
## 3.7 Phase S (取引安全性 - 詐欺対策強化) — v1.5 で新設

**スコープ**: 16-23h (約 1 週間)、8 commits 構造、Phase M の後に直列実行
...
- ❌ **「Trust 高い側が後発送」ルール**を撤回
- 撤回理由:
  - 物理的制約無視 (仕事/学校で発送日が限定される人を不利化)
  - 新規ユーザー差別 (Trust ゼロは常に先発送)
  - 戦略 2-5 違反 (Trust=権利化)
- 詳細は教訓 7.8 参照
...
#### Phase S-2: 発送フロー (3 commits, 約 7-9h)
- **C-S2**: 発送日合意制 UI + DB ロジック (3-4h)
- **C-S3**: 発送写真証跡 + 追跡番号入力 (両者並行) (2-3h)
- **C-S4**: 詐欺通報機能 (1-2h)
...
| 2 | EXIF データ検証精度 (C-S3) | ★ | 段階的緩和、ユーザー報告で改善 |
| 3 | 補償基金料率調整 | ★ | β データで Phase 2 で決定 |

### 着手前 spike (1-2h)

- **pg_cron の動作確認** (テスト環境で migration + 関数 + scheduled job)
- Phase M の Realtime spike と並行可能
...
| **b** | 追跡番号の偽造 (別人宛番号) | △ 配送 API なしだと検出不可、受領者報告 + 手動確認 | β 必須 (C-S3) |
...
| Q4 (工数 16-23h) | **Yes** | C-S7 が +1h (pg_cron 初導入)、C-S8 が +1h (詐欺パターン検証) |
...
### 4.0 β リリース前必須項目 (累計 87-127h、約 11-16 日) — v1.5 で更新

β 開始 (2026-06) までに**必ず**完了する項目。v1.5 で Phase S + 法的整備を追加。
...
| 4 | **Phase S 詐欺対策** (発送日合意 + 証跡 + 自動監視 + 仲裁) ← v1.5 新設 | **16-23h** | **Phase S (新設)** |
